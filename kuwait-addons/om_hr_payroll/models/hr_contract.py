...
class HrContract(models.Model):
    ...
    nsbeh_not_worked = fields.Integer('%نسبة خصم عدم الدوام',required=True)

    add_salary_ids = fields.One2many('add.salary','hr_contract_id',string="اضافة رواتب")
...
class AddSalary(models.Model):
    ...
    name = fields.Char('ملاحظة', required=True)
    amount = fields.Float('قيمة الزيادة',required=True)
    hr_contract_id = fields.Many2one('hr.contract',string="hr_contract")
    state = fields.Selection([('draft', 'مسودة'), ('posted', 'مرحل')], default='draft',copy=False,readonly=True)



    def add_salary_amount(self):
        self.hr_contract_id.wage = self.hr_contract_id.wage + self.amount
        self.state = 'posted'
    def cancel_salary_amount(self):
        self.hr_contract_id.wage = self.hr_contract_id.wage - self.amount
        self.state = 'draft'
    def unlink(self):
        if self.state == 'draft':
            raise UserError(_('لا يمكنك الحذف '))
        return super(AddSalary, self).unlink()
