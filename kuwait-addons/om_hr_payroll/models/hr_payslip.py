...
class HrPayslip(models.Model):
    ...
    date_to = fields.Date(string='Date To', readonly=True, required=True,
        ...
        states={'draft': [('readonly', False)]})
    net_slary_new = fields.Float(string='الراتب المستحق', readonly=True,  compute='_compute_net_slary_new', store=True , copy=False,)
    ghyab_slary_new = fields.Float(string='خصومات الغياب', readonly=True,  compute='_compute_net_slary_new', store=True , copy=False,)
    ...
    @api.depends('worked_days_line_ids','worked_days_line_ids.number_of_days','worked_days_line_ids.contract_id')
    def _compute_net_slary_new(self):
        for me in self:
            d1 = datetime.strptime(str(me.date_from), "%Y-%m-%d")
            d2 = datetime.strptime(str(me.date_to), "%Y-%m-%d")
            days = (d2 - d1).days
            print ("days",days)

            not_work = (days - me.worked_days_line_ids.number_of_days )
            amount_in_day = ((me.worked_days_line_ids.contract_id.wage)/30)
            nsbeh_not_worked = me.contract_id.nsbeh_not_worked
            khsem = (not_work) * (amount_in_day*(nsbeh_not_worked/100))

            me.ghyab_slary_new = khsem
            me.net_slary_new = me.worked_days_line_ids.contract_id.wage - khsem
